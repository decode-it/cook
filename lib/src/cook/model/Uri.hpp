...
namespace cook { namespace model { 
    ...
    public:
        ...
        static Uri recipe_uri(const std::string & uri_str)
        {
            Uri d;
            gubg::Strange strange(uri_str);
            d.absolute_ = strange.pop_if('/');
            std::string part;
            while (strange.pop_until(part, '/'))
                d.add_path_part(part);
            d.set_name(strange.str());
            return d;
        }

        static Uri book_uri(const std::string & uri_str)
        {
            Uri d;
            gubg::Strange strange(uri_str);
            d.absolute_ = strange.pop_if('/');
        ...
        {
            return name_ < rhs.name_ && std::lexicographical_compare(path_.begin(), path_.end(), rhs.path_.begin(), rhs.path_.end());
        }
