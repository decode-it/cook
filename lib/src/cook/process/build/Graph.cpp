...
Result Graph::add_edge(vertex_descriptor consumer, vertex_descriptor producer, EdgeType type)
{
    ...
    MSS(consumer_label.which() !=  producer_label.which());

    const FileLabel * lbl = boost::get<FileLabel>(&producer_label);
    if (!!lbl)
    {
        auto p = boost::in_edges(producer, g_);

        MSG_MSS(p.first == p.second, InternalError, "file " << *lbl << " is already generated by another command");
    }
...
Result Graph::topological_commands(std::vector<vertex_descriptor> & commands) const
{
    ...
    {
        const Label & l = g_[vd];
        const CommandLabel * lbl = boost::get<CommandLabel>(&l);
