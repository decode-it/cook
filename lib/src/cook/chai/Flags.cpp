...
#define L_CASE(TYPE) case Flag::TYPE: return str << #TYPE
    ...
    Result Flags::exactly(const std::initializer_list<Flag> & lst) const
    {
        MSS_BEGIN(Result);
        auto s = cook::log::scope("Check flags", [&](auto & n) {
                                  for(Flag f : lst)
                                  n.attr("required & allowed", f);
                                  });
        ...
        for(auto fl : lst)
        {
            unsigned int mask = (1u << static_cast<unsigned int>(fl));
            MSG_MSS( (flags & mask) == mask, Error, "Flag " << fl << " should be set");
            flags &= (~mask);
        }

        for(unsigned int i = 0; i < static_cast<unsigned int>(Flag::_End); ++i)
        {
            unsigned int mask = (1u << i);
            MSG_MSS((flags & mask) == 0, Error, "Flag " << static_cast<Flag>(i) << " should not be set");
        }
