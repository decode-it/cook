...
private:
    ...
    void add_library_(std::ostream &ofs, const model::Recipe & recipe) const;
    void add_executable_(std::ostream &ofs, const model::Recipe & recipe) const;
    void add_interface_(std::ostream & ofs, const model::Recipe & recipe) const;

    std::string recipe_name_(const model::Recipe & recipe) const;
    std::string deduce_library_type_(const model::Recipe & recipe) const;

    void add_source_and_header_(std::ostream & ofs, const model::Recipe & recipe, bool add_headers) const;
    void set_target_properties_(std::ostream & ofs, const model::Recipe & recipe, const std::string &keyword) const;
    void set_link_paths_(std::ostream & oss, const model::Recipe & recipe) const;
